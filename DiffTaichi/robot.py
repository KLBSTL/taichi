...
def optimize(toi, visualize):
    ...
    np.savez(
        f"robot_{options.robot_id}_weights.npz",
...
parser.add_argument('task', type=str, help='train/plot/test')
...
def main():
    ...
    if options.task == 'plot':
        ...
        load_weights(f"robot_{options.robot_id}_weights.npz")
        clear()
        forward('test{}'.format(options.robot_id),visualize=True)
    else:
