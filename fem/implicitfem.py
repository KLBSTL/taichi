...
@ti.kernel
def get_indices():
    # calculate all meshes on surface
    cnt = 0
    for c in F_vertices:
        if c % 5 != 4:
    ...
    for i in range(cnt):
        ...
        v1 = indices[i * 3 + 1]
        v2 = indices[i * 3 + 2]
        edge_indices[i * 3] = ti.Vector([v0, v1])
        edge_indices[i * 3 + 1] = ti.Vector([v1, v2])
        edge_indices[i * 3 + 2] = ti.Vector([v2, v0])
...
def main():
    ...
    if args.gui == "ggui":
        ...
        while gui.running:
            ...
            # project vertices
            vertices_2d = T(F_x.to_numpy() / 3)
            indices_np = indices.to_numpy()

            # draw triangle faces
            for i in range(0, len(indices_np), 3):
                if indices_np[i] == 0 and indices_np[i + 1] == 0 and indices_np[i + 2] == 0:
                    break  # 遇到未填充的索引，停止绘制
                v0_idx = indices_np[i]
                v1_idx = indices_np[i + 1]
                v2_idx = indices_np[i + 2]
                gui.triangle(vertices_2d[v0_idx], vertices_2d[v1_idx], vertices_2d[v2_idx], color=0xEECCAA)

            # draw edges (wireframe)
            edge_np = edge_indices.to_numpy()
            for i in range(edge_np.shape[0]):
                e0 = edge_np[i][0]
                e1 = edge_np[i][1]
                gui.line(vertices_2d[e0], vertices_2d[e1], radius=1, color=0x0000FF)
