...
def substep():
    ...
    for i in ti.grouped(x):
        ...
        if dist <= ball_radius + 0.01:
            normal = offset_to_center / dist
            # F = kdn − c(v ⋅ n)n
            force += 0.2 * spring_Y * offset_to_center - damping * (x[i] - x_last[i]).dot(normal) * normal
    ...
    #     if dist <= ball_radius + 0.01:
    #         normal = offset_to_center / dist
    #         force += 0.1 * spring_Y * normal - damping * (x[i] - x_last[i]).dot(normal) * normal
    #         x[i] = x[i] + (x[i] - x_last[i]) * damping + force * dt * dt / mass
    #         x_last[i] = x_buffer
