...
r_record = ti.Vector.field(3,dtype=float,shape=(n,n))
r1 = ti.Vector.field(3,dtype=float,shape=(n,n))
    ...
    for i in grouped(x):
        for spring_offset in ti.static(spring_offsets):
            ...
            if 0 <= j[0] < n and 0 <= j[1] < n:
                ...
                # x_p[i] -= corre
                # x_p[j] += corre
                ti.atomic_add(x_p[i][0], -corre[0])
                ti.atomic_add(x_p[i][1], -corre[1])
                ti.atomic_add(x_p[i][2], -corre[2])

                ti.atomic_add(x_p[j][0], corre[0])
                ti.atomic_add(x_p[j][1], corre[1])
                ti.atomic_add(x_p[j][2], corre[2])
